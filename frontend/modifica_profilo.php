...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (empty($errori)) {
    if ($foto_profilo_new) {
      $sql = "UPDATE dottori SET nome=?, cognome=?, biografia=?, data_inizio_lavoro=?, ospedale=?, specializzazione=?, foto_profilo=?
                    WHERE id_dottore=?";
      $stmt = $connessione->prepare($sql);
      $stmt->execute([$nome, $cognome, $biografia, $data_inizio_lavoro, $ospedale, $specializzaz, $foto_profilo_new, $_SESSION['utente']]);
    } else {
      $sql = "UPDATE dottori SET nome=?, cognome=?, biografia=?, data_inizio_lavoro=?, ospedale=?, specializzazione=?
                    WHERE id_dottore=?";
      $stmt = $connessione->prepare($sql);
      $stmt->execute([$nome, $cognome, $biografia, $data_inizio_lavoro, $ospedale, $specializzaz, $_SESSION['utente']]);
    }
    $successo = true;
  }
}

$sql = "SELECT dottori.nome, dottori.cognome, dottori.biografia, dottori.data_inizio_lavoro,
               dottori.data_nascita, dottori.sesso, dottori.foto_profilo,
  ...
  <div class="page-wrapper">
    ...
    <form method="POST" action="modifica_profilo.php" enctype="multipart/form-data">
      ...
      <div class="form-card">
        ...
        <div class="form-grid">
          <div class="field">
            <label class="field-label" for="data_inizio_lavoro">Data inizio lavoro</label>
            <input type="date" id="data_inizio_lavoro" name="data_inizio_lavoro"
              value="<?= htmlspecialchars($dottore['data_inizio_lavoro'] ?? '') ?>">
          </div>
          <div class="field">
